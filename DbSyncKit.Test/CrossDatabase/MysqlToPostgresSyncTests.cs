...
namespace DbSyncKit.Test.CrossDatabase
{
    ...
    public class MysqlToPostgresSyncTests
    {
        ...
        public MysqlToPostgresSyncTests()
        {
            Source = new DbSyncKit.MySQL.Connection("localhost", 3306, "SourceChinook", "root", "");
            Destination = new DbSyncKit.PostgreSQL.Connection("localhost", 5432, "destinationChinook", "postgres", ""); 
        ...
        private void DataSync<T>() where T : IDataContract
        {
            ...
            stopwatch.Restart();
            var query = Sync.QueryBuilder.GetSqlQueryForSyncData(data, Sync.ContractFetcher.DestinationQueryGenerationManager!);
            stopwatch.Stop();
